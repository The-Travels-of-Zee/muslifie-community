...
import { createLikeNotification } from "./notificationActions";
...
// Get only the number of posts saved by a user
export async function getUserSavedPostsCount(userId) {
  try {
    if (!userId) {
    ...
    // Query all "saves" across posts where userRef matches
    ...
    return { count: savesSnapshot.size };
  } catch (error) {
...
export async function togglePostLike(postId, user, isLiked) {
  try {
    ...
    if (isLiked && likeDoc.exists) {
      ...
      const postOwnerId = postData.userRef?.id; // <-- Firestore doc id of post owner
      if (postOwnerId && postOwnerId !== user.id) {
        await createLikeNotification({
          postId,
          likerId: user.id,
          postOwnerId,
          likerName: user.fullName || user.email?.split("@")[0] || "Someone",
          postTitle: postData.title || postData.content?.substring(0, 50) || "your post",
          profileImage: user.profileImage,
        });
      }
    }
...
export async function togglePostSave(postId, user, isSaved) {
  try {
    ...
    await getUserSavedPostsCount(user.id);
...
// ✅ Toggle report/unreport a post
export async function toggleReportPost(postId, currentUserId, reason = "inappropriate_content") {
  try {
    ...
    const existingReport = await adminDb
      ...
      .where("reportedPostRef", "==", postRef)
      .where("reportedByRef", "==", userRef)
      .get();

    if (!existingReport.empty) {
      // ✅ Unreport (delete existing report)
      const batch = adminDb.batch();
      existingReport.forEach((doc) => batch.delete(doc.ref));
      await batch.commit();

      return { success: true, isReported: false, message: "Post unreported" };
    }

    // ✅ Report (create new)
    await adminDb.collection("reported-posts").add({
      reportedPostRef: postRef,
    ...
    return { success: true, isReported: true, message: "Post reported" };
  } catch (error) {
    console.error("Error toggling report for post:", error);
    return { success: false, error: "Failed to toggle report for post" };
  }
}

// ✅ Check if a post is already reported by user
export async function checkUserReportedPost(postId, currentUserId) {
  try {
    if (!postId || !currentUserId) return { isReported: false };
    ...
    return { isReported: !reportSnap.empty };
  } catch (error) {
