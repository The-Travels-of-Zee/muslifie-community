...
export async function getUserLikedPosts(userId, limit = 10, lastLikeId = null) {
  try {
    console.log("Getting liked posts for userId:", userId);
    ...
    // First, we need to get all posts to check their likes subcollections
    // This is a complex query since we need to search across subcollections
    ...
    // Check each post for likes by this user
    for (const postDoc of postsSnapshot.docs) {
      const likesQuery = adminDb
        .collection("posts")
        .doc(postDoc.id)
        .collection("likes")
      ...
      if (!likesSnapshot.empty) {
        ...
        if (postData.userRef) {
          try {
            ...
            console.error(`Failed to fetch user for post ${postDoc.id}:`, err);
          }
        }

        // Count comments
        const commentsSnapshot = await adminDb.collection("posts").doc(postDoc.id).collection("comments").get();
    ...
    likedPostsData.sort((a, b) => new Date(b.likedAt) - new Date(a.likedAt));

    // Apply pagination
    let paginatedData = likedPostsData;
    if (lastLikeId) {
      const lastIndex = likedPostsData.findIndex((item) => item.likeId === lastLikeId);
      if (lastIndex !== -1) {
        paginatedData = likedPostsData.slice(lastIndex + 1);
      }
    }

    const result = paginatedData.slice(0, limit);
    const hasMore = paginatedData.length > limit;
    const newLastLikeId = result.length > 0 ? result[result.length - 1].likeId : null;

    console.log("Liked posts loaded:", result.length);
...
// Alternative more efficient approach using collectionGroup query
