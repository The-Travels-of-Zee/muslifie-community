...
// Apple Sign In with Firebase ID token
export async function signInWithApple(identityToken, userType = "traveler", email, fullName) {
  try {
    ...
    // Set auth cookies (your existing function)
    if (data.token) {
      await setAuthCookie(data.token, data.user);
    }

    // Save/update user in Firestore (your existing function)
    if (data.user) {
      await saveUserToFirestore(data.user);
    }
